...
const float diffRate = 0.001f; // a multiplyer used to alter the amount of diffusion that takes place per time step
const float fluidAddedOnClick = 0.2f; // a multiplyer to alter the amount of density that is added per click
const float velocityAddedOnClick = 5.0f; // a multiplyer to alter the amount of velocity that is added per click
const int brushSize = 1; // the distance that the brush will spread from the centre cell ( if the brush size is one it will cover a 3 x 3 grid )
